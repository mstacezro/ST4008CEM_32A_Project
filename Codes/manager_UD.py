...
def update_func():
    ...
    c = conn.cursor()
    c.execute("SELECT * FROM Manager WHERE Manager_id=?", (delete_box.get(),))
    if c.fetchall:
...
myLabel = Label(editor_frame, image=converted_image, text="EDIT Manager DATA", font=(
...
def delete():
    ...
    c.execute("DELETE FROM Manager WHERE Manager_id=?", (delete_box.get(),))
...
myLabel = Label(editor_frame1, image=converted_image1,
                text="EDIT Manager DATA", font=('Arial', '20', 'bold'), compound='left')
...
def edit_delete():
    ...
    c.execute("DELETE FROM Manager WHERE Status='active' ")
    conn.commit()
    conn.close()
